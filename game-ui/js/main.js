...
$(window).load(function() {
    ...
    function drawDebug() {
        ...
        for (var k = 0; input_stream.length > 0 && k < input_stream.length; k++) {
            ...
            // k is bot id. horrible var name!
            for (var i = 0; input_stream[k].length > 0 && turn_no < input_stream[k].length && i < input_stream[k][turn_no].length; i++) {
        ...
        for (var j = 0; j < LEGEND.length; j++) {
            for (var i = 0; val_stream[j].length > 0 && turn_no < val_stream[j].length && i < val_stream[j][turn_no].length; i++) {

                data += "<p class='val'>" + val_stream[j][turn_no][i] + "</p>";
            }
            for (var i = 0; ign_stream[j].length > 0 && turn_no < ign_stream[j].length && i < ign_stream[j][turn_no].length; i++) {

                data += "<p class='ign'>" + ign_stream[j][turn_no][i] + "</p>";
            }
            for (var i = 0; inv_stream[j].length > 0 && turn_no < inv_stream[j].length && i < inv_stream[j][turn_no].length; i++) {
        ...
        for (var i = 0; notifications.length > 0 && turn_no < notifications.length && i < notifications[turn_no].length; i++) {
            data_n += "<div>";
